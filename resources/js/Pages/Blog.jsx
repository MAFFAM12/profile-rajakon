        ...
        <div className="rounded-2xl overflow-hidden bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 shadow-sm hover:shadow-lg transition-all duration-300 h-full flex flex-col">
            ...
            <div className="p-5 flex flex-col flex-1">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    {blog.published_at && (
                        <span className="text-xs text-zinc-400">{blog.published_at}</span>
                    )}
                </div>
...
const Blog = ({ blogs, kategoris = [], aktifKategori }) => {
    const { data, links, meta } = blogs;

    const filterKategori = (kat) => {
        router.get('/blog', kat ? { kategori: kat } : {}, { preserveState: true });
    };
                        ...
                        <div className="flex flex-wrap gap-2 mb-10">
                            ...
                            {kategoris.map((kat) => (
                                <button
                                    ...
                                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${aktifKategori === kat
                                        ? 'bg-zinc-900 dark:bg-white text-white dark:text-zinc-900'
                                        : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200'
                                        }`}
                                >
